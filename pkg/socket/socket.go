package socket

import (
	"errors"
	"fmt"
	"io"
	"log"

	"golang.org/x/net/websocket"
)

type empty struct{}

type Packet struct {
	Type string         `json:"type"`
	Data map[string]any `json:"data"`
}

type Socket struct {
	subscribers map[string][]func(args ...any)

	ws *websocket.Conn
	id string

	onConnect    func()
	onDisconnect func()

	ackFns map[int]func(...any)
	ackID  int

	closedCh chan empty
}

func New(ws *websocket.Conn, id string) *Socket {
	s := &Socket{
		subscribers: map[string][]func(args ...any){},

		ws: ws,
		id: id,

		onConnect:    nil,
		onDisconnect: nil,

		ackFns: map[int]func(...any){},
		ackID:  1,

		closedCh: make(chan empty),
	}

	go s.onPacketReceived()

	return s
}

func (s *Socket) emit(event string, args ...any) {
	for _, fn := range s.subscribers[event] {
		fn(args...)
	}
}

func (s *Socket) on(event string, fn func(args ...any)) {
	s.subscribers[event] = append(s.subscribers[event], fn)
}

func (s *Socket) onPacketReceived() {
	for {
		var pkt Packet
		if err := websocket.JSON.Receive(s.ws, &pkt); err != nil {
			if errors.Is(err, io.EOF) {
				s.disconnect(errors.New("user disconnected"))
			} else {
				s.disconnect(fmt.Errorf("user disconnected with the error: %s", err.Error()))
			}
			break
		}

		switch pkt.Type {
		case "ack-from-client":
			id, ok := pkt.Data["id"].(float64)
			if !ok {
				log.Printf("invalid id type: %v", pkt.Data["id"])
				continue
			}
			ackID := int(id)

			args, ok := pkt.Data["args"].([]any)
			if !ok {
				log.Printf("invalid args type: %v", pkt.Data["args"])
				continue
			}
			if ackFn, ok := s.ackFns[ackID]; ok {
				ackFn(args...)
				delete(s.ackFns, ackID)
			}
		case "ack-from-server":
			event, ok := pkt.Data["event"].(string)
			if !ok {
				log.Printf("invalid event type: %v", pkt.Data["event"])
				continue
			}
			id, ok := pkt.Data["id"].(float64)
			if !ok {
				log.Printf("invalid id type: %v", pkt.Data["id"])
				continue
			}
			args, ok := pkt.Data["args"].([]any)
			if !ok {
				log.Printf("invalid args type: %v", pkt.Data["args"])
				continue
			}

			ackFn := func(args ...any) {
				s.emitAckFromServer(int(id), args...)
			}
			args = append(args, ackFn)
			s.emit(event, args...)
		case "event":
			event, ok := pkt.Data["event"].(string)
			if !ok {
				log.Printf("invalid event type: %v", pkt.Data["event"])
				continue
			}
			args, ok := pkt.Data["args"].([]any)
			if !ok {
				log.Printf("invalid args type: %v", pkt.Data["args"])
				continue
			}
			s.emit(event, args...)
		}
	}
}

func (s *Socket) connect() error {
	err := websocket.JSON.Send(s.ws, Packet{
		Type: "connect",
		Data: map[string]any{
			"id": s.id,
		},
	})
	if err != nil {
		return fmt.Errorf("socket: sending connect packet: %w", err)
	}

	s.emit("connect")

	if s.onConnect != nil {
		// Avoid blocking
		go s.onConnect()
	}

	<-s.closedCh
	return nil
}

func (s *Socket) OnConnect(fn func()) {
	s.onConnect = fn
}

func (s *Socket) disconnect(err error) error {
	defer close(s.closedCh)

	if s.onDisconnect != nil {
		// Avoid blocking
		go s.onDisconnect()
	}

	reason := err.Error()
	err = websocket.JSON.Send(s.ws, Packet{
		Type: "disconnect",
		Data: map[string]any{
			"reason": reason,
		},
	})
	if err != nil {
		return fmt.Errorf("socket: sending disconnect packet: %w", err)
	}

	s.emit("disconnect", reason)

	if err := s.ws.Close(); err != nil {
		return fmt.Errorf("socket: close websocket: %w", err)
	}
	return nil
}

func (s *Socket) OnDisconnect(fn func()) {
	s.onDisconnect = fn
}

func (s *Socket) ID() string {
	return s.id
}

func (s *Socket) Emit(event string, args ...any) error {
	if ackFn, err := GetAckFunc(args); err == nil {
		ackID := s.ackID
		s.ackID += 1
		s.ackFns[ackID] = ackFn

		// Remove the "ack" function
		args = argRemoveLast(args)

		err := websocket.JSON.Send(s.ws, Packet{
			Type: "ack-from-client",
			Data: map[string]any{
				"event": event,
				"id":    ackID,
				"args":  ensureArgs(args),
			},
		})
		if err != nil {
			return fmt.Errorf("socket: calling emit with ack from client: %w", err)
		}
		return nil
	}

	err := websocket.JSON.Send(s.ws, Packet{
		Type: "event",
		Data: map[string]any{
			"event": event,
			"args":  ensureArgs(args),
		},
	})
	if err != nil {
		return fmt.Errorf("socket: calling emit: %w", err)
	}
	return nil
}

func (s *Socket) emitAckFromServer(id int, args ...any) error {
	err := websocket.JSON.Send(s.ws, Packet{
		Type: "ack-from-server",
		Data: map[string]any{
			"id":   id,
			"args": ensureArgs(args),
		},
	})
	if err != nil {
		return fmt.Errorf("socket: calling emit with ack from server: %w", err)
	}
	return nil
}

func (s *Socket) On(event string, fn func(args ...any)) {
	s.on(event, fn)
}

func ensureArgs(args []any) []any {
	if len(args) == 0 {
		return []any{}
	}
	return args
}
